...
class EcmAddress {
  EcmAddress(  {  required this.id,    required this.type,    required this.street1,    required this.street2,    required this.city,    required this.state,    required this.zipcode,    required this.country,    required this.contactPhone,    required this.fullName,    required this.userId,    required this.createdDate,    required this.createdBy,    required this.createdById,     this.updatedDate,     this.updatedBy,     this.updatedById,     this.deletedDate,     this.deletedBy,     this.deletedById,    required this.isDeleted } );
  String id;
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is EcmAddress&&
    other.id == id&&
    other.type == type&&
  ...
  /// Returns a new [EcmAddress] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EcmAddress? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EcmAddress[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EcmAddress[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<EcmAddress> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EcmAddress>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EcmAddress.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EcmAddress> mapFromJson(dynamic json) {
    final map = <String, EcmAddress>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EcmAddress.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EcmAddress-objects as value to a dart map
  static Map<String, List<EcmAddress>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EcmAddress>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EcmAddress.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
...
extension EcmAddressJsonUtils on EcmAddress{
static List<EcmAddress> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EcmAddress>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
