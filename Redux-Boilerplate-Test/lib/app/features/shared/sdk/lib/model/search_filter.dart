...
class SearchFilter {
  SearchFilter(  {  required this.offset,    required this.limit,    required this.skip,    required this.order,    required this.fields,    required this.include } );
  ///
  ...
  @override
  String toString() => 'SearchFilter[offset=$offset, limit=$limit, skip=$skip, order=$order, fields=$fields, include=$include]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SearchFilter[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SearchFilter[$key]" has a null value in JSON.');
        });
        return true;
      }());
...
extension SearchFilterJsonUtils on SearchFilter{
static List<SearchFilter> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SearchFilter>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
