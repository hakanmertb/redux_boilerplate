...
class Search {
  ...
  /// Returns a new [Search] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Search? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Search[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Search[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Search> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Search>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Search.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Search> mapFromJson(dynamic json) {
    final map = <String, Search>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Search.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Search-objects as value to a dart map
  static Map<String, List<Search>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Search>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Search.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
...
extension SearchJsonUtils on Search{
static List<Search> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Search>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
