...
class NewSlot {
  NewSlot(  {  required this.day,    required this.timeSlot,    required this.isAvailable,    required this.parentId,    required this.createdDate,    required this.createdBy,    required this.createdById,     this.updatedDate,     this.updatedBy,     this.updatedById,     this.deletedDate,     this.deletedBy,     this.deletedById,    required this.isDeleted,    required this.searchId,    required this.providerId,    required this.bookId } );
  ///
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is NewSlot&&
    other.day == day&&
  ...
  @override
  String toString() => 'NewSlot[day=$day, timeSlot=$timeSlot, isAvailable=$isAvailable, parentId=$parentId, createdDate=$createdDate, createdBy=$createdBy, createdById=$createdById, updatedDate=$updatedDate, updatedBy=$updatedBy, updatedById=$updatedById, deletedDate=$deletedDate, deletedBy=$deletedBy, deletedById=$deletedById, isDeleted=$isDeleted, searchId=$searchId, providerId=$providerId, bookId=$bookId]';
  ...
  /// Returns a new [NewSlot] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static NewSlot? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "NewSlot[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "NewSlot[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<NewSlot> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <NewSlot>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = NewSlot.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, NewSlot> mapFromJson(dynamic json) {
    final map = <String, NewSlot>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = NewSlot.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of NewSlot-objects as value to a dart map
  static Map<String, List<NewSlot>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<NewSlot>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = NewSlot.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
...
extension NewSlotJsonUtils on NewSlot{
static List<NewSlot> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <NewSlot>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
