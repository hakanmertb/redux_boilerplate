//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:my_api_client/api.dart';
import 'package:test/test.dart';

// tests for EcmOrderItemWithRelations
void main() {
  // final instance = EcmOrderItemWithRelations();

  group('test EcmOrderItemWithRelations', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // num quantity
    test('to test the property `quantity`', () async {
      // TODO
    });

    // num priceAtOrder
    test('to test the property `priceAtOrder`', () async {
      // TODO
    });

    // DateTime createdDate
    test('to test the property `createdDate`', () async {
      // TODO
    });

    // String createdBy
    test('to test the property `createdBy`', () async {
      // TODO
    });

    // String createdById
    test('to test the property `createdById`', () async {
      // TODO
    });

    // DateTime updatedDate
    test('to test the property `updatedDate`', () async {
      // TODO
    });

    // String updatedBy
    test('to test the property `updatedBy`', () async {
      // TODO
    });

    // String updatedById
    test('to test the property `updatedById`', () async {
      // TODO
    });

    // DateTime deletedDate
    test('to test the property `deletedDate`', () async {
      // TODO
    });

    // String deletedBy
    test('to test the property `deletedBy`', () async {
      // TODO
    });

    // String deletedById
    test('to test the property `deletedById`', () async {
      // TODO
    });

    // bool isDeleted
    test('to test the property `isDeleted`', () async {
      // TODO
    });

    // String productId
    test('to test the property `productId`', () async {
      // TODO
    });

    // String orderId
    test('to test the property `orderId`', () async {
      // TODO
    });

    // EcmProductWithRelations product
    test('to test the property `product`', () async {
      // TODO
    });

    // Object foreignKey
    test('to test the property `foreignKey`', () async {
      // TODO
    });

    // EcmOrderWithRelations order
    test('to test the property `order`', () async {
      // TODO
    });


  });

}
