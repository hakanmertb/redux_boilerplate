...
class ReviewPartial {
  /// Returns a new [ReviewPartial] instance.
  ReviewPartial({
    this.id,
    this.rate,
    this.review,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ReviewPartial &&
    other.id == id &&
    other.rate == rate &&
    other.review == review &&
  ...
  @override
  String toString() => 'ReviewPartial[id=$id, rate=$rate, review=$review, likeCount=$likeCount, dislikeCount=$dislikeCount, replyText=$replyText, email=$email, name=$name, parentId=$parentId, createdDate=$createdDate, createdBy=$createdBy, createdById=$createdById, updatedDate=$updatedDate, updatedBy=$updatedBy, updatedById=$updatedById, deletedDate=$deletedDate, deletedBy=$deletedBy, deletedById=$deletedById, isDeleted=$isDeleted, providerId=$providerId, userId=$userId]';
  ...
  /// Returns a new [ReviewPartial] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ReviewPartial? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ReviewPartial[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ReviewPartial[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ReviewPartial> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ReviewPartial>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ReviewPartial.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ReviewPartial> mapFromJson(dynamic json) {
    final map = <String, ReviewPartial>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ReviewPartial.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ReviewPartial-objects as value to a dart map
  static Map<String, List<ReviewPartial>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ReviewPartial>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ReviewPartial.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
