...
class Provider {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Provider[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Provider[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Provider> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Provider>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Provider.listFromJson(
          entry.value,
          growable: growable,
        );
      }
