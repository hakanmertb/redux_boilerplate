...
class Parent {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Parent[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Parent[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Parent> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <Parent>[];
  ...
  static Map<String, List<Parent>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Parent.listFromJson(
          entry.value,
          growable: growable,
        );
      }
