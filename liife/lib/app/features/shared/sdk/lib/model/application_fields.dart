//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class ApplicationFields {
    /// Returns a new [ApplicationFields] instance.
  ApplicationFields(
    { this.id,
    this.type,
    this.zipcode,
    this.gender,
    this.birthday,
    this.heightFeet,
    this.heightInch,
    this.weight,
    this.householdSize,
    this.qualifyingLifeEvent,
    this.smoking,
    this.houseHoldIncomeRange,
    this.healthConditions,
    this.pregnancy,
    this.sampleFieldName,
    this.createdDate,
    this.createdBy,
    this.createdById,
    this.updatedDate,
    this.updatedBy,
    this.updatedById,
    this.deletedDate,
    this.deletedBy,
    this.deletedById,
    this.isDeleted,
    this.userId, }
  );

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? id;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? type;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? zipcode;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? gender;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? birthday;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? heightFeet;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? heightInch;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? weight;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? householdSize;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? qualifyingLifeEvent;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? smoking;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? houseHoldIncomeRange;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? healthConditions;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? pregnancy;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? sampleFieldName;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? createdDate;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? createdBy;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? createdById;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? updatedDate;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? updatedBy;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? updatedById;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? deletedDate;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? deletedBy;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? deletedById;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? isDeleted;

  ///
  /// Please note: This property should have been non-nullable! Since the specification file
  /// does not include a default value (using the "default:" property), however, the generated
  /// source code must fall back to having a nullable type.
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  bool? userId;



  @override
  bool operator ==(Object other) => identical(this, other) || other is ApplicationFields&&
    other.id == id&&
    other.type == type&&
    other.zipcode == zipcode&&
    other.gender == gender&&
    other.birthday == birthday&&
    other.heightFeet == heightFeet&&
    other.heightInch == heightInch&&
    other.weight == weight&&
    other.householdSize == householdSize&&
    other.qualifyingLifeEvent == qualifyingLifeEvent&&
    other.smoking == smoking&&
    other.houseHoldIncomeRange == houseHoldIncomeRange&&
    other.healthConditions == healthConditions&&
    other.pregnancy == pregnancy&&
    other.sampleFieldName == sampleFieldName&&
    other.createdDate == createdDate&&
    other.createdBy == createdBy&&
    other.createdById == createdById&&
    other.updatedDate == updatedDate&&
    other.updatedBy == updatedBy&&
    other.updatedById == updatedById&&
    other.deletedDate == deletedDate&&
    other.deletedBy == deletedBy&&
    other.deletedById == deletedById&&
    other.isDeleted == isDeleted&&
    other.userId == userId;

  @override
  int get hashCode => 
    // ignore: unnecessary_parenthesis
    (id == null ? 0 : id!.hashCode) +
    (type == null ? 0 : type!.hashCode) +
    (zipcode == null ? 0 : zipcode!.hashCode) +
    (gender == null ? 0 : gender!.hashCode) +
    (birthday == null ? 0 : birthday!.hashCode) +
    (heightFeet == null ? 0 : heightFeet!.hashCode) +
    (heightInch == null ? 0 : heightInch!.hashCode) +
    (weight == null ? 0 : weight!.hashCode) +
    (householdSize == null ? 0 : householdSize!.hashCode) +
    (qualifyingLifeEvent == null ? 0 : qualifyingLifeEvent!.hashCode) +
    (smoking == null ? 0 : smoking!.hashCode) +
    (houseHoldIncomeRange == null ? 0 : houseHoldIncomeRange!.hashCode) +
    (healthConditions == null ? 0 : healthConditions!.hashCode) +
    (pregnancy == null ? 0 : pregnancy!.hashCode) +
    (sampleFieldName == null ? 0 : sampleFieldName!.hashCode) +
    (createdDate == null ? 0 : createdDate!.hashCode) +
    (createdBy == null ? 0 : createdBy!.hashCode) +
    (createdById == null ? 0 : createdById!.hashCode) +
    (updatedDate == null ? 0 : updatedDate!.hashCode) +
    (updatedBy == null ? 0 : updatedBy!.hashCode) +
    (updatedById == null ? 0 : updatedById!.hashCode) +
    (deletedDate == null ? 0 : deletedDate!.hashCode) +
    (deletedBy == null ? 0 : deletedBy!.hashCode) +
    (deletedById == null ? 0 : deletedById!.hashCode) +
    (isDeleted == null ? 0 : isDeleted!.hashCode) +
    (userId == null ? 0 : userId!.hashCode);

  @override
  String toString() => 'ApplicationFields[id=$id, type=$type, zipcode=$zipcode, gender=$gender, birthday=$birthday, heightFeet=$heightFeet, heightInch=$heightInch, weight=$weight, householdSize=$householdSize, qualifyingLifeEvent=$qualifyingLifeEvent, smoking=$smoking, houseHoldIncomeRange=$houseHoldIncomeRange, healthConditions=$healthConditions, pregnancy=$pregnancy, sampleFieldName=$sampleFieldName, createdDate=$createdDate, createdBy=$createdBy, createdById=$createdById, updatedDate=$updatedDate, updatedBy=$updatedBy, updatedById=$updatedById, deletedDate=$deletedDate, deletedBy=$deletedBy, deletedById=$deletedById, isDeleted=$isDeleted, userId=$userId]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (this.id != null) {
      json[r'id'] = this.id;
    }
    if (this.type != null) {
      json[r'type'] = this.type;
    }
    if (this.zipcode != null) {
      json[r'zipcode'] = this.zipcode;
    }
    if (this.gender != null) {
      json[r'gender'] = this.gender;
    }
    if (this.birthday != null) {
      json[r'birthday'] = this.birthday;
    }
    if (this.heightFeet != null) {
      json[r'heightFeet'] = this.heightFeet;
    }
    if (this.heightInch != null) {
      json[r'heightInch'] = this.heightInch;
    }
    if (this.weight != null) {
      json[r'weight'] = this.weight;
    }
    if (this.householdSize != null) {
      json[r'householdSize'] = this.householdSize;
    }
    if (this.qualifyingLifeEvent != null) {
      json[r'qualifyingLifeEvent'] = this.qualifyingLifeEvent;
    }
    if (this.smoking != null) {
      json[r'smoking'] = this.smoking;
    }
    if (this.houseHoldIncomeRange != null) {
      json[r'houseHoldIncomeRange'] = this.houseHoldIncomeRange;
    }
    if (this.healthConditions != null) {
      json[r'healthConditions'] = this.healthConditions;
    }
    if (this.pregnancy != null) {
      json[r'pregnancy'] = this.pregnancy;
    }
    if (this.sampleFieldName != null) {
      json[r'sampleFieldName'] = this.sampleFieldName;
    }
    if (this.createdDate != null) {
      json[r'createdDate'] = this.createdDate;
    }
    if (this.createdBy != null) {
      json[r'createdBy'] = this.createdBy;
    }
    if (this.createdById != null) {
      json[r'createdById'] = this.createdById;
    }
    if (this.updatedDate != null) {
      json[r'updatedDate'] = this.updatedDate;
    }
    if (this.updatedBy != null) {
      json[r'updatedBy'] = this.updatedBy;
    }
    if (this.updatedById != null) {
      json[r'updatedById'] = this.updatedById;
    }
    if (this.deletedDate != null) {
      json[r'deletedDate'] = this.deletedDate;
    }
    if (this.deletedBy != null) {
      json[r'deletedBy'] = this.deletedBy;
    }
    if (this.deletedById != null) {
      json[r'deletedById'] = this.deletedById;
    }
    if (this.isDeleted != null) {
      json[r'isDeleted'] = this.isDeleted;
    }
    if (this.userId != null) {
      json[r'userId'] = this.userId;
    }
    return json;
  }

  /// Returns a new [ApplicationFields] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ApplicationFields? fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();

      // Ensure that the map contains the required keys.
      // Note 1: the values aren't checked for validity beyond being non-null.
      // Note 2: this code is stripped in release mode!
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ApplicationFields[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ApplicationFields[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ApplicationFields(
        id: mapValueOfType<bool>(json, r'id'),
        type: mapValueOfType<bool>(json, r'type'),
        zipcode: mapValueOfType<bool>(json, r'zipcode'),
        gender: mapValueOfType<bool>(json, r'gender'),
        birthday: mapValueOfType<bool>(json, r'birthday'),
        heightFeet: mapValueOfType<bool>(json, r'heightFeet'),
        heightInch: mapValueOfType<bool>(json, r'heightInch'),
        weight: mapValueOfType<bool>(json, r'weight'),
        householdSize: mapValueOfType<bool>(json, r'householdSize'),
        qualifyingLifeEvent: mapValueOfType<bool>(json, r'qualifyingLifeEvent'),
        smoking: mapValueOfType<bool>(json, r'smoking'),
        houseHoldIncomeRange: mapValueOfType<bool>(json, r'houseHoldIncomeRange'),
        healthConditions: mapValueOfType<bool>(json, r'healthConditions'),
        pregnancy: mapValueOfType<bool>(json, r'pregnancy'),
        sampleFieldName: mapValueOfType<bool>(json, r'sampleFieldName'),
        createdDate: mapValueOfType<bool>(json, r'createdDate'),
        createdBy: mapValueOfType<bool>(json, r'createdBy'),
        createdById: mapValueOfType<bool>(json, r'createdById'),
        updatedDate: mapValueOfType<bool>(json, r'updatedDate'),
        updatedBy: mapValueOfType<bool>(json, r'updatedBy'),
        updatedById: mapValueOfType<bool>(json, r'updatedById'),
        deletedDate: mapValueOfType<bool>(json, r'deletedDate'),
        deletedBy: mapValueOfType<bool>(json, r'deletedBy'),
        deletedById: mapValueOfType<bool>(json, r'deletedById'),
        isDeleted: mapValueOfType<bool>(json, r'isDeleted'),
        userId: mapValueOfType<bool>(json, r'userId'),
      );
    }
    return null;
  }

  static List<ApplicationFields> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ApplicationFields>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ApplicationFields.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
    }
    return result.toList(growable: growable);
  }

  static Map<String, ApplicationFields> mapFromJson(dynamic json) {
    final map = <String, ApplicationFields>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ApplicationFields.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
    }
    return map;
  }

  // maps a json object with a list of ApplicationFields-objects as value to a dart map
  static Map<String, List<ApplicationFields>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ApplicationFields>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ApplicationFields.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

  /// The list of required keys that must be present in a JSON.
  static const requiredKeys = <String>{
  };
}

extension ApplicationFieldsJsonUtils on ApplicationFields{
static List<ApplicationFields> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ApplicationFields>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ApplicationFields.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
    }
    return result.toList(growable: growable);
  }}
