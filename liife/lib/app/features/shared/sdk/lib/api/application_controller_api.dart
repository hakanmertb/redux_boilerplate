...
class ApplicationControllerApi {
  ...
  /// Performs an HTTP 'GET /applications/{id}/user' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] id (required):
  Future<Response> applicationControllerFindUserWithHttpInfo(String id,) async {
    // ignore: prefer_const_declarations
    final path = r'/applications/{id}/user'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [String] id (required):
  Future<User?> applicationControllerFindUser(String id,) async {
    final response = await applicationControllerFindUserWithHttpInfo(id,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
